	...
	for action in $ACTIONS; do
		for mode in $MODE; do
			...
			if [ ! -z "$CONTENT" ]; then
				...
				bash "$WORK_PATH/../send2telegram/photo.sh" "$HOME/log/${action}-${mode}.png" "$COMPUTER ${location} $PACKAGE cb-manager $SPEAKER ${action} - ${mode}"
			fi
		done
	done

	echo "$1 - Restart"
	bash "$WORK_PATH/service.sh" "$1" stop
	bash "$WORK_PATH/service.sh" "$1" start
else
